...
const validateCred = (array) => {
    const checkNum = array[array.length - 1];
    const reversedArray = [];
    const everyOther = [];
    const theRest = [];
    const doubled = [];
    ...
    for (i = 0; i <= reversedArray.length - 1; i++) {
        if (i % 2 !== 0) {
            everyOther.push(reversedArray[i]);
        } else {
            theRest.push(reversedArray[i]);
        }
    }

    everyOther.forEach(every => {
        if ((every * 2) <= 9) {
            doubled.push((every * 2));
        } else {
            doubled.push((every * 2) - 9);
        }
    })

    let sum1 = doubled.reduce((a,b) => {
        return a + b;
    });

    let sum2 = theRest.reduce((a,b) => {
        return a + b;
    });
